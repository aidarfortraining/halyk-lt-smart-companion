...
class Chip:
    ...
    needs_address: bool = False  # choosing this chip → ask the client to type a stay address
...
class Step:
    ...
    requires_address: bool = False      # step active only while trip.address_pending (typed-address step)
    text_input: bool = False    # interactive via free text instead of chips
    input_hint: str = ""        # placeholder shown for a text_input step
...
PHASE_0 = [
    Step(
        key="welcome", phase=0, silent=True,
        emits=[
            {"as": "ai",
             "text": "Здравствуйте! Ваши билеты успешно приобретены. Маршрут: Алматы → Астана, "
                     "ночной поезд (~13 часов): отправление 4 июня в 20:00, прибытие 5 июня в 09:00, "
                     "4 пассажира, 38 000 ₸. Меня зовут Halyk Travel AI — я Ваш персональный ассистент "
                     "по поездке и помогу организовать её на каждом этапе: от бронирования жилья "
                     "до возвращения домой."},
        ],
    ),
    Step(
        key="hotel", phase=0, plan_item="hotel",
        concern="Где остановимся в Астане? На эти даты жильё разбирают быстро",
        ai_prompt=("Билеты уже куплены. Сначала объясни ценность бронирования жилья заранее: на эти "
                   "даты места в Астане разбирают быстро, а онлайн-бронь гарантирует номер и обычно "
                   "даёт бесплатную отмену. Отель удобно забронировать через Booking.com, и при оплате "
                   "брони картой Halyk начисляются бонусы, которыми можно покрыть часть других расходов. "
                   "Не называй конкретные отели. Заверши вопросом, где клиент планирует остановиться."),
        fallback=("Теперь предлагаю определиться с жильём — на эти даты места в Астане разбирают "
                  "быстро. Отель удобно забронировать онлайн через Booking.com: это гарантирует "
                  "номер на ваши даты, а при оплате брони картой Halyk начисляются бонусы, которыми "
                  "можно покрыть часть других расходов поездки. Где Вы планируете остановиться?"),
        chips=[
            ...
            Chip(label="🏠 Апартаменты", sub="укажу адрес проживания", value="appart",
                 plan_value="Апартаменты · Airba Fresh активирован", plan_tag="g",
                 budget_category="Отель", budget_fact=35000, is_apartments=True, needs_address=True),
            Chip(label="✓ Уже забронировал", sub="укажу адрес проживания", value="booked",
                 plan_value="Уже забронировано ✓", plan_tag="g",
                 budget_category="Отель", budget_fact=48000, needs_address=True),
        ],
    ),
    Step(
        key="stay_address", phase=0, requires_address=True, text_input=True,
        input_hint="Впишите адрес проживания…",
        concern="Куда именно едем с вокзала? Нужен точный адрес",
        ai_prompt=("Клиент выбрал апартаменты или уже готовую бронь. Попроси указать точный адрес "
                   "проживания в Астане и объясни, что он нужен, чтобы построить маршрут трансфера "
                   "от вокзала и подобрать сервисы рядом. Один-два предложения."),
        fallback=("Пожалуйста, укажите точный адрес места, где Вы планируете остановиться. Он нужен, "
                  "чтобы построить маршрут трансфера от вокзала и подсказать сервисы рядом с Вами."),
    ),
    Step(
    ...
    Step(
        ...
        ai_prompt=("Клиент выбрал жильё. Предложи оформить туристическую страховку на семью. Сначала "
                   "объясни, зачем она нужна: покрывает медицинскую помощь в поездке, в том числе "
                   "детям, и защищает от непредвиденных расходов в дороге. Затем перечисли три "
                   "варианта: Базовая — только медицина (1 500 ₸), Семейная — медицина и багаж с "
                   "повышенными бонусами (3 000 ₸), Премиум — всё включено (5 000 ₸). Упомяни, что "
                   "данные семьи уже заполнены и оформление займёт пару минут."),
        fallback=("Пока не забыли — предлагаю оформить туристическую страховку на семью. Она покрывает "
                  "медицинскую помощь в поездке, в том числе детям, и защищает от непредвиденных "
                  "расходов в дороге. Есть три варианта: Базовая — только медицина (1 500 ₸), Семейная — "
                  "медицина и багаж с повышенными бонусами (3 000 ₸) и Премиум — всё включено (5 000 ₸). "
                  "Данные семьи уже заполнены, оформление займёт пару минут."),
        chips=[
...
PHASE_1 = [
    Step(
        ...
        ai_prompt=("За 7 дней до поездки. Объясни, зачем заранее собрать дорожную аптечку: в ночном "
                   "поезде ~13 часов с детьми 5 и 9 лет под рукой нужны жаропонижающее, пластыри и "
                   "средство от укачивания, и лучше подготовить их дома, а не искать в дороге. Затем "
                   "предложи готовый набор «Семья в дорогу» от Appteka с доставкой домой и отметь, что "
                   "обычная доставка заранее дешевле экспресса в поездке."),
        fallback=("За неделю до поездки предлагаю собрать дорожную аптечку. В ночном поезде ~13 часов "
                  "с детьми пригодятся жаропонижающее, пластыри и средство от укачивания — лучше иметь "
                  "всё под рукой заранее, чем искать в пути. Appteka соберёт готовый набор «Семья в "
                  "дорогу» и привезёт домой, а обычная доставка дешевле экспресса в поездке."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("За 3 дня. Объясни, зачем заранее продумать трансфер с вокзала: по прибытии с "
                   "детьми и чемоданами в дождь добираться самостоятельно неудобно, а такси довезёт "
                   "прямо до места (вокзал Нурлы Жол → {hotel}, ~15 минут). Отметь, что бронировать "
                   "заранее не нужно — добавим напоминание и вызовем одним тапом в день приезда."),
        fallback=("Предлагаю заранее продумать трансфер с вокзала. По прибытии в Астану с детьми и "
                  "чемоданами в дождь добираться самостоятельно неудобно — такси довезёт прямо до "
                  "места: Нурлы Жол → {hotel}, ~15 минут. Бронировать заранее не нужно: добавим "
                  "напоминание и вызовем одним тапом в день приезда."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("Суббота дождливая, {sat}, дети 5 и 9 лет. Объясни, зачем заранее выбрать занятие "
                   "под крышей: в дождь активности на улице отпадают, и лучше спланировать день, чем "
                   "решать на месте с уставшими детьми. Затем предложи Думан — крытый аттракционный "
                   "комплекс, где детям интересно в любую погоду; билеты удобно купить через Kino.kz "
                   "без комиссии, картой Halyk — с бонусами."),
        fallback=("В субботу ожидается дождь, {sat}, поэтому предлагаю заранее выбрать занятие для "
                  "детей под крышей — так не придётся решать на месте в плохую погоду. Думан — крытый "
                  "аттракционный комплекс, где дети 5 и 9 лет проведут день независимо от дождя. Билеты "
                  "удобно купить через Kino.kz без комиссии, картой Halyk — с бонусами."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("Семья приедет в пятницу утром после ночного поезда — уставшая. Объясни, зачем "
                   "позаботиться об ужине заранее: искать кафе на ходу с уставшими детьми не хочется, "
                   "а бронь столика снимает этот вопрос. Затем предложи сервис Halyk Рестораны — "
                   "заведения рядом с {hotel} с детским меню, со столиком на пятницу; оплата на месте."),
        fallback=("Предлагаю заранее позаботиться об ужине в пятницу. После ночного поезда искать кафе "
                  "на ходу с уставшими детьми не хочется — лучше забронировать столик заранее. В сервисе "
                  "Halyk Рестораны есть заведения рядом с {hotel} с детским меню; оплата — на месте."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("В субботу дождь {sat}, дети 5 и 9 лет. Объясни, зачем подготовить непромокаемую "
                   "экипировку заранее: без зонтов и дождевиков прогулка в дождь обернётся промокшими "
                   "детьми, а подготовиться проще заблаговременно. Затем предложи зонты и детские "
                   "дождевики через Halyk Market с доставкой к {hotel} и рассрочкой 0-0-6."),
        fallback=("В субботу ожидается дождь, {sat}, поэтому предлагаю заранее подготовить детям "
                  "непромокаемую экипировку — иначе прогулка обернётся промокшими детьми. Зонты и "
                  "детские дождевики удобно заказать через Halyk Market с доставкой к {hotel} и "
                  "рассрочкой 0-0-6."),
        chips=[
...
PHASE_2 = [
    ...
    Step(
        ...
        ai_prompt=("Семья едет в апартаменты, через 40 минут Астана. Объясни, зачем заказать продукты "
                   "заранее: в апартаментах нет завтрака, а идти в магазин с детьми и чемоданами сразу "
                   "после поезда неудобно. Затем предложи Airba Fresh — доставит продукты прямо к "
                   "приезду, пока семья в пути."),
        fallback=("Через 40 минут Астана. В апартаментах нет завтрака, а идти в магазин с детьми и "
                  "чемоданами сразу после поезда неудобно — поэтому предлагаю заказать продукты "
                  "заранее. Airba Fresh успеет доставить их прямо к Вашему приезду."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("Через 10 минут Астана, клиент стоит в проходе с детьми и чемоданами. Напомни про "
                   "заранее запланированный трансфер: чтобы не искать машину и не стоять в очереди на "
                   "вокзале, такси можно вызвать заранее — маршрут уже готов (вокзал Нурлы Жол → "
                   "{hotel}). Один тап, и машина уже едет."),
        fallback=("Через 10 минут Астана. Чтобы не стоять с детьми и чемоданами в очереди на вокзале, "
                  "предлагаю сразу вызвать такси — маршрут уже готов: вокзал Нурлы Жол → {hotel}. "
                  "Один тап, и машина уже едет."),
        chips=[
...
PHASE_3 = [
    ...
    Step(
        ...
        ai_prompt=("Суббота 10:30, до Думана 30 минут, дождь и пробки. Объясни, почему сейчас удобнее "
                   "такси: с детьми в дождь идти пешком некомфортно, а машина довезёт прямо ко входу. "
                   "Предложи вызвать такси одним тапом."),
        fallback=("До Думана около 30 минут, на улице дождь и пробки. С детьми в такую погоду пешком "
                  "некомфортно — предлагаю вызвать такси, оно довезёт прямо ко входу."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("Воскресенье, последний день в Астане. Объясни, зачем подумать о сувенирах заранее: "
                   "перед ночным поездом не придётся искать подарки впопыхах, а доставка экономит время "
                   "в последний день. Затем предложи сладости Рахат и шоколад Баян Сулу через Halyk "
                   "Market с доставкой прямо к {hotel}."),
        fallback=("Последний день в Астане — предлагаю заранее выбрать сувениры близким, чтобы не "
                  "искать подарки впопыхах перед поездом. Сладости Рахат и шоколад Баян Сулу можно "
                  "заказать через Halyk Market с доставкой прямо к {hotel}."),
        chips=[
    ...
    Step(
        ...
        ai_prompt=("Воскресенье 19:00 — пора на вокзал к ночному поезду. Объясни, зачем выехать "
                   "заранее на такси: с детьми и багажом важно спокойно успеть к отправлению, без "
                   "спешки и поиска машины. Предложи вызвать такси {hotel} → вокзал Нурлы Жол одним тапом."),
        fallback=("Воскресенье 19:00 — пора выдвигаться на вокзал к ночному поезду. Чтобы спокойно "
                  "успеть к отправлению с детьми и багажом, предлагаю вызвать такси {hotel} → "
                  "вокзал Нурлы Жол."),
        chips=[
...
def _step_active(trip, step):
    """A step is inactive if it needs a branch we're not on:
    apartments-only steps in hotel-path, or the typed-address step when no address is pending."""
    if step.requires_apartments and not trip.is_apartments:
        return False
    if step.requires_address and not trip.address_pending:
        return False
    return True


def active_step(trip):
    """The interactive step currently awaiting the client, or None (not persisted; §4)."""
