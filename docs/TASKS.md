...
> (`docker compose up` → `localhost:8000`). Тесты: backend 9/9 (pytest), frontend 2/2 (vitest).
...
>
> **Пост-сборка (после вех 0–10):**
> - **Сброс/возобновление.** `POST /api/trip/reset` (удалить Trip → пересоздать seed → запустить первый шаг). Обновление страницы возобновляет компаньон (идемпотентный `start` + флаг `localStorage.halyk_purchased`); кнопка ↻ в шапке чата сбрасывает поездку и возвращает к визарду. Тест `test_reset_replays_from_scratch`.
> - **Визард покупки билета (3 шага).** `frontend/src/components/wizard/Wizard.tsx` (+ `styles/wizard.css`, scoped `.wizard-root`) — порт шагов 1–3 из `prototype.html` V1, статичный, без AI, контент под seed-сценарий (5–7 июня, ночной поезд, 38 000 ₸). «Купить билет» → компаньон. Строка «Кто едет» — display-only (модалку с просроченными документами убрали; дифференциатор раскрывается в компаньоне). Браузерный прогон визард→покупка→компаньон выполнен (2026-06-05), 0 ошибок в консоли.
> - **Онбординг и адрес проживания (2026-06-05).** Первое сообщение компаньона разнесено: silent-шаг `welcome` (билеты куплены + данные + описание ассистента), затем `hotel`-вопрос (Booking + бонусы, без названий отелей). Выбор «Апартаменты»/«Уже забронировал» → текстовый шаг `stay_address` (поле `Trip.address_pending`, миграция `0002`; приём свободного текста; `input_hint` в снапшоте; `Chips` прячется, `InputArea` показывает подсказку). Адрес сохраняется в `hotel_name`/`hotel_address`. Тесты `test_phase0_welcome_then_hotel`, `test_phase0_address_flow`. Полный Playwright-прогон (визард → booking-путь → Итоги 175 000/169 500 🎯 и отдельно апарт-ветка с вводом адреса) — 0 ошибок в консоли.
> - **Тон и подача LLM (2026-06-05).** Официальный стиль на «Вы»; схема «объясни сервис (что/зачем/польза) → предложи»; без нагнетания; `max_tokens` 200→400 (длинные объяснения больше не обрываются). Тексты всех сервисных шагов переписаны. `graph/llm.py` + `graph/steps.py`.
>
